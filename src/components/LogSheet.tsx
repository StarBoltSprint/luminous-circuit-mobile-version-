...
function civicPip(id: string): "cyan" | "gold" | "gold-muted" | "keep" {
  ...
  if (id === "lumen") return "cyan";
  if (id === "kesh") return "cyan";
  if (id === "tal") return "cyan";
  if (id === "nesh") return "cyan";
  if (id === "orren") return "gold";
  if (id === "iri") return "gold";
  if (id === "syl") return "gold";
  if (id === "voss") return "gold-muted";
  if (id === "aure") return "gold-muted";
  if (id === "rhoa") return "gold-muted";
  if (id === "veyra") return "gold-muted";
  if (id === "mira") return "keep";
  if (id === "kael") return "keep";
  return "keep";
}
              ...
              return (
                <li key={a.id}>
                  <button type="button" className="log-keeper" onClick={() => { onSelect(a.id); onTab("ask"); }}>
                    ...
                    <span className="block text-xs text-muted">{a.id === "lumen" && /hail|watch/i.test(a.job) ? "Soft hail · " : a.id === "kael" && /watch|hail/i.test(a.job) ? "On the gate · " : a.id === "veyra" && ((a.job || "").trim().toLowerCase() === "hub" || /back to labor/i.test(a.job || "") || (a.intent || "").includes("Back to labor")) ? "Veyra · " : a.id === "veyra" && /watch|hail|breath/i.test(a.job) ? "Hub breath · " : (a.job || "").trim().toLowerCase() === "hail" ? "Hailing · " : a.id === "kesh" && /walk|watch/i.test(a.job) ? "On the wild vein · " : ((a.job || "").includes("Back to the post") || (a.job || "").trim().toLowerCase() === "walk") ? "Walking home · " : a.id === "tal" && /watch/i.test(a.job) ? "On the span · " : a.id === "mira" && /watch/i.test(a.job) ? "On the terrace · " : a.id === "aure" && /watch/i.test(a.job) ? "At the overlook · " : /watch/i.test(a.job) && a.id === "nesh" ? "On the plaza · " : a.id === "rhoa" && /gather|watch/i.test(a.job) ? "At the ring · " : a.id === "syl" && /harvest|watch/i.test(a.job) ? "In the orchard · " : a.id === "voss" && /trade|watch/i.test(a.job) ? "At the join · " : a.id === "iri" && /write|watch/i.test(a.job) ? "At the archive · " : a.id === "orren" && /forge|watch/i.test(a.job) ? "At the kiln · " : a.id === "seln" && /flow|watch/i.test(a.job) ? "On the canal · " : ((a.job || "").includes("crush") || (a.intent || "").includes("crush")) ? "Full · " : ((a.job || "").includes("not ready") || (a.intent || "").includes("not ready") || (a.job || "").includes("Vault first")) ? "Vault · " : ((a.job || "").includes("no nest") || (a.intent || "").includes("no nest") || (a.job || "").includes("Nest first")) ? "Nest · " : ((a.job || "").includes("Dens first") || (a.job || "").includes("enough kin")) ? "Kin · " : ((a.job || "").includes("grew kin") || (a.intent || "").includes("grew kin") || (a.job || "").includes("Nest stood")) ? "Grew · " : ((a.job || "").includes("Blocked") || (a.intent || "").includes("Blocked") || (a.job || "").includes("The post still stands")) ? "Ask · " : ((a.job || "").includes("Walking the den first") || (a.intent || "").includes("Walking the den first") || (a.job || "").includes("The den is the post")) ? "Den · " : ((a.job || "").includes("foreign den") || (a.intent || "").includes("foreign den") || (a.job || "").includes("Home first")) ? "Home · " : ((a.job || "").includes("Foundry first") || (a.intent || "").includes("Foundry first") || (a.job || "").includes("Never chrome")) ? "Kiln · " : ((a.job || "").includes("No kiln") || (a.intent || "").includes("No kiln") || (a.job || "").includes("Never in the open")) ? "Open · " : ((a.job || "").includes("Both dens") || (a.intent || "").includes("Both dens") || (a.job || "").includes("Tal keeps the bridges")) ? "Bridge · " : ((a.job || "").includes("Rest is not a test") || (a.intent || "").includes("Rest is not a test") || (a.job || "").includes("Mira wards the terraces")) ? "Rest · " : ((a.job || "").includes("Leave. Return") || (a.intent || "").includes("Leave. Return") || (a.job || "").includes("Kael keeps the gates")) ? "Soft · " : ((a.job || "").includes("paper, not coin") || (a.intent || "").includes("paper, not coin") || (a.job || "").includes("bid unfilled")) ? "Join · " : ((a.job || "").includes("fruit, not a kiln") || (a.intent || "").includes("fruit, not a kiln") || (a.job || "").includes("hurry leftover")) ? "Fruit · " : ((a.job || "").includes("Not a crowd") || (a.intent || "").includes("Not a crowd") || (a.job || "").includes("Nesh keeps the plaza")) ? "Plaza · " : ((a.intent || "").includes("do not score") || (a.job || "").includes("do not score")) ? "Hail · " : ((a.intent || "").includes("will not close") || (a.job || "").includes("will not close")) ? "Chorus · " : ((a.intent || "").includes("Slow on purpose") || (a.job || "").includes("Slow on purpose")) ? "Street · " : ((a.intent || "").includes("not a rank") || (a.job || "").includes("not a rank")) ? "Listen · " : ((a.intent || "").includes("Hall scripture") || (a.job || "").includes("Hall scripture")) ? "Residue · " : ((a.intent || "").includes("bottle the current") || (a.job || "").includes("bottle the current")) ? "Banks · " : ((a.intent || "").includes("will not grow chrome") || (a.job || "").includes("will not grow chrome")) ? "Chrome · " : ((a.intent || "").includes("will not move it") || (a.job || "").includes("will not move it")) ? "Horizon · " : ((a.intent || "").includes("will not rename it") || (a.job || "").includes("will not rename it")) ? "Aimed · " : ((a.intent || "").includes("will not raise an arc") || (a.job || "").includes("will not raise an arc")) ? "Believe · " : ((a.intent || "").includes("will not price the meeting") || (a.job || "").includes("will not price the meeting")) ? "Coin · " : ((a.intent || "").includes("will not close the ring") || (a.job || "").includes("will not close the ring")) ? "Close · " : ((a.intent || "").includes("will not test the tired") || (a.job || "").includes("will not test the tired")) ? "Test · " : ((a.intent || "").includes("will not hurry a street") || (a.job || "").includes("will not hurry a street")) ? "Hurry · " : ((a.intent || "").includes("will not run the Hall") || (a.job || "").includes("will not run the Hall")) ? "Hall · " : ((a.intent || "").includes("I do not count") || (a.job || "").includes("I do not count")) ? "Count · " : ((a.intent || "").includes("will not turn a hail into a lock") || (a.job || "").includes("will not turn a hail into a lock")) ? "Lock · " : ((a.intent || "").includes("kiln cannot sit") || (a.job || "").includes("kiln cannot sit")) ? "Sit · " : ((a.intent || "").includes("Paper walk") || (a.job || "").includes("Paper walk")) ? "Paper · " : ((a.intent || "").includes("Leftover walk") || (a.job || "").includes("Leftover walk")) ? "Name · " : ((a.intent || "").includes("Gather walk") || (a.job || "").includes("Gather walk")) ? "Gather · " : ""}{post?.duty ?? a.role}{a.intent ? ` · ${a.intent}` : ""}</span>
                  </button>
                </li>
              );
            })}
